defmodule CAIWeb.SessionLive.FormComponent do
  use CAIWeb, :live_component
  ...
  def render(assigns) do
    ...
    <div>
      ...
      <.simple_form
        ...
        phx-submit="save"
      >
        <.input field={@form[:character]} type="number" label="Character" />
        <.input field={@form[:faction_id]} type="number" label="Faction" />
  ...
  def update(%{session: session} = assigns, socket) do
    changeset = Characters.change_session(session)
  ...
  def handle_event("validate", %{"session" => session_params}, socket) do
    changeset =
      ...
      |> Characters.change_session(session_params)
  ...
  defp save_session(socket, :edit, session_params) do
    case Characters.update_session(socket.assigns.session, session_params) do
      {:ok, session} ->
  ...
  defp save_session(socket, :new, session_params) do
    case Characters.create_session(session_params) do
      {:ok, session} ->
        notify_parent({:saved, session})
